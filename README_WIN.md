...
### Рабочая схема: автологин + запертый экран

1. **Задача в Планировщике** (от администратора):
   ...
   Скрипт создаёт три задачи:
   - `cvetopt-autostart` — поднимает `cvetopt.bat` при входе сервисной учётки;
   - `cvetopt-autostart-watchdog` — каждые 5 минут проверяет `/api/state` и, если сервер не отвечает (упал, завис при старте, кто-то случайно закрыл окно), поднимает его заново. «При входе» срабатывает один раз за вход и само по себе не восстанавливает работу после сбоя — этим и занимается watchdog;
   - `cvetopt-autostart-lock` — запирает экран через 2 минуты после входа.

   Сервер при запертом экране продолжает работать — приложение управляет Excel только через COM, эмуляции клавиатуры нигде нет, поэтому блокировка ему не мешает.

2. **Auto-logon сервисной учётки.** Скачайте [Sysinternals Autologon](https://learn.microsoft.com/en-us/sysinternals/downloads/autologon), запустите, введите имя/пароль сервисной учётки → Enable. После перезагрузки система логинится сама, задача поднимает сервер, экран запирается.

Эксплуатация:

- **Никогда не выходите из системы (Log off) под сервисной учёткой** — сервер живёт внутри её сессии и умрёт вместе с ней. Отключайтесь (Disconnect). Если всё же вышли — достаточно снова войти, задача сработает по триггеру входа.
- В сессии сервисной учётки висит окно консоли `cvetopt - do not close this window`. Обычные пользователи его не видят (экран заперт), но если вы зашли по RDP под этой учёткой — не закрывайте его.

### Запасной вариант: без автологина и без окна

Если автологин по какой-то причине неприемлем, задачу можно повесить на загрузку системы — тогда вход в систему не нужен вообще и окна нет:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\register-startup-task.ps1 -UserId "SERVER\invoice" -Unattended
```

Скрипт спросит пароль учётки (Windows хранит его в Планировщике в зашифрованном виде).

**Это компромисс с заметным риском, а не равноценная альтернатива:**

- Excel уезжает в сессию 0, где Microsoft его автоматизацию не поддерживает. `DisplayAlerts = False` выставлен не во всех модулях, поэтому модальное окно Excel («восстановить файл?», «файл занят») — реальный сценарий. В сессии 0 такое окно **никто не увидит и не закроет**: прогон зависнет навсегда, а планировщик расписания будет молча пропускать все последующие запуски.
- Кнопки **«выбрать папку/файл» в Настройках перестают работать** — системный диалог открывается на невидимом рабочем столе сессии 0. Пути вводите вручную.
- Через сессию 0 пойдут и ручные нажатия кнопок, а не только автозапуск.

### Автозапуск кнопок по расписанию (`schedule:` в `config.yaml`)

Приложение умеет само нажимать часть кнопок по расписанию (например, «Скачать отчёты» по четвергам в 9:00) — настройки в `config.yaml`, секция `schedule:` (день недели, час, минута, `enabled` на всё и на каждую задачу отдельно).

Это работает, только пока запущен сам сервер (uvicorn) — планировщик расписания живёт внутри него. `cvetopt.exe` **останавливает сервер при закрытии окна**, но только если сам его и поднял — если сервер уже был поднят раньше (например, задачей из Планировщика выше), `cvetopt.exe` просто открывает окно поверх него и **не трогает сервер** при закрытии. Поэтому:

- Без задачи из Планировщика: расписание сработает, только если кто-то держит `cvetopt.exe` открытым в момент срабатывания (вт/чт 9:00). Если в этот момент сервер не запущен — задача на эту неделю просто пропускается, без повторной попытки.
- С задачей из Планировщика (см. выше): сервер работает постоянно в фоне, расписание срабатывает само по себе, а `cvetopt.exe` можно открывать и закрывать когда угодно — на автоматизацию это не влияет.
