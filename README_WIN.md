...
- Без задачи из Планировщика (шаг 2 выше): расписание сработает, только если кто-то держит `cvetopt.exe` открытым в момент срабатывания (вт/чт 9:00). Если в этот момент сервер не запущен — задача на эту неделю просто пропускается, без повторной попытки.
- С задачей из Планировщика: сервер работает постоянно в фоне, расписание срабатывает само по себе, а `cvetopt.exe` можно открывать и закрывать когда угодно — на автоматизацию это не влияет.
