...
1. **Задача в Планировщике** (от администратора):
   ...
   Скрипт создаёт две задачи: `cvetopt-autostart` (поднимает `cvetopt.bat` при входе сервисной учётки) и `cvetopt-autostart-lock` (запирает экран через 2 минуты после входа). Сервер при запертом экране продолжает работать — приложение управляет Excel только через COM, эмуляции клавиатуры нигде нет, поэтому блокировка ему не мешает.
