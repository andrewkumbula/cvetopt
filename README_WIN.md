...
**`cvetopt.exe`** — обычная программа Windows (ярлык на рабочий стол):

1. Поднимает сервер **в фоне** (без чёрного окна).
2. Открывает окно **cvetopt** (Edge/Chrome в режиме приложения).
3. Пока окно открыто — можно жать кнопки на вкладке «Запуск».
4. **Закрыли окно — сервер останавливается.**

**Сборка exe** (один раз на сервере после `git pull`):

```powershell
cd C:\Apps\cvetopt
powershell -ExecutionPolicy Bypass -File scripts\build-launcher-exe.ps1
```

Или **двойной клик** по `build-cvetopt-exe.bat` в папке проекта (важно: не из `C:\Users\...`, а из каталога cvetopt).
...
Файл `cvetopt.exe` лежит в корне проекта рядом с `cvetopt.bat`. Запасной вариант без сборки — `cvetopt-launcher.vbs` (то же поведение, но выглядит как скрипт).

Принудительно остановить сервер: `cvetopt-stop.bat`.

### Две учётки на одном сервере (админ + пользователь)

Сервер слушает `127.0.0.1:8000` на **всём ПК**. Поэтому:

| Что видите | Почему |
|------------|--------|
| Админ запустил — у пользователя «открывается» | Пользователь просто подключается к **уже запущенному** серверу админа |
| Пользователь сам не может запустить | Часто нет прав на `C:\Apps\cvetopt`, нет `uv`/Python в PATH, ярлык только на столе админа |

**Как надо:**

1. Выдать пользователю права (от администратора, подставьте имя учётки):

```powershell
powershell -ExecutionPolicy Bypass -File C:\Apps\cvetopt\scripts\grant-user-access.ps1 -User "ИМЯ_ПК\ИмяПользователя"
```

2. Ярлык должен быть на **рабочем столе пользователя** (`C:\Users\Имя\Desktop\cvetopt.lnk`), не на столе админа.

3. `uv` и Python — в **системном** PATH (для всех), не только в профиле админа. Проверка под учёткой пользователя:
...
4. Работать должен **пользователь**: он запускает ярлык, он закрывает окно. Админ не держит сервер «для него» — иначе Excel/архивы идут под чужой учёткой (Access Denied на `C:\Invoice`).

5. Одновременно — **один** cvetopt. Если админ уже держит порт 8000, лаунчер пользователя только откроет окно к чужому процессу; при закрытии окна пользователя `cvetopt-stop` убьёт сервер админа.
