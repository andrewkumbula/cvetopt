...
Есть два варианта. Начните с варианта Б (проще в эксплуатации), и если Excel-шаги под ним зависают — переходите на вариант А.

### Вариант А: автологин + видимое окно
...
Задачу из шага 2 можно создать одной командой (от администратора), вместо ручных кликов по GUI:

```powershell
cd C:\Apps\cvetopt
powershell -ExecutionPolicy Bypass -File scripts\register-startup-task.ps1 -UserId "SERVER\invoice" -Interactive
```

Скрипт вешает задачу на `cvetopt.bat` (как в шаге 2 выше) — при входе появится окно консоли и сам откроется браузер, это нормально. **Окно закрывать нельзя** — это и есть сервер.

### Вариант Б: без автологина и без окна

Задача стартует при загрузке системы от имени сервисной учётки, скрыто, входить в систему никому не нужно:
...
Плата за удобство — две оговорки:

- **Excel COM без интерактивной сессии официально не поддерживается.** Часто работает, но может и зависнуть без внятной ошибки. После настройки обязательно прогоните вручную любой шаг с Excel (например, «Шаблон → копия на сегодняшнюю дату») и убедитесь, что он завершается. Если зависает — перерегистрируйте задачу с `-Interactive` и настройте автологин (вариант А).
...
- С задачей из Планировщика (вариант А или Б выше): сервер работает постоянно в фоне, расписание срабатывает само по себе, а `cvetopt.exe` можно открывать и закрывать когда угодно — на автоматизацию это не влияет.
