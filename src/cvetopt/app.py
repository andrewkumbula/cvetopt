...
async def api_runtime_settings_update(request: Request) -> JSONResponse:
    ...
    try:
        ...
        ecu_arch_err = validate_ecuador_archive_dir(
            env,
            settings.ecuador_archive_dir,
            settings.ecuador_output_dir,
            runtime=settings,
        )
        if ecu_arch_err:
            return JSONResponse({"error": ecu_arch_err}, status_code=422)
...
@app.post("/run/gypsophila-split")
async def run_gypsophila_split_route(request: Request):
    """Biflorica → «… Гипсофила.xlsx» и «… Роза.xlsx» (аналог макроса Гипсофила + Да)."""
    busy = _reject_if_busy()
    if busy is not None:
        return busy
    env = EnvSettings()
